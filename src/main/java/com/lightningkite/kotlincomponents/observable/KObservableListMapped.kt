 ...
 */
class KObservableListMapped<S, E>(val source: KObservableListInterface<S>, val mapper: (S) -> E, val reverseMapper: (E) -> S) : KObservableListInterface<E> {
    ...
    override val onUpdate = source.onUpdate.mapObservable<KObservableListInterface<S>, KObservableListInterface<E>>({ it -> this@KObservableListMapped }, { throw IllegalAccessException() })
    override val onReplace: MutableSet<(KObservableListInterface<E>) -> Unit> get() = source.onReplace.map({ input -> { input(this) }})
}

fun <S, E> KObservableListInterface<S>.mapObservableList(mapper: (S) -> E, reverseMapper: (E) -> S):KObservableListMapped<S, E> = KObservableListMapped(this, mapper, reverseMapper)
fun <S, E> KObservableListInterface<S>.mapObservableList(mapper: (S) -> E):KObservableListMapped<S, E> = KObservableListMapped(this, mapper, {throw IllegalArgumentException()})